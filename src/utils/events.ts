...
export function hookInternalLinkMouseEventHandlers(
	app: App,
...
export function isMouseEventExternal(evt: MouseEvent, el: HTMLElement) {
	return (
		!evt.relatedTarget ||
		(isTargetElement(evt, evt.relatedTarget) && !el.contains(evt.relatedTarget))
	);
}

// Taken from app.js
export function isTypable(el: Node) {
	return el.nodeName === "INPUT" || (el.instanceOf(HTMLElement) && el.contentEditable === "true");
}
...
export function isTargetElement(evt: UIEvent, target: EventTarget | null): target is Element {
	return (
		!!target && (target instanceof Element || instanceofInWindow(target, evt.win, "Element"))
	);
}
 ...
 * @returns
 */
export function isTargetHTMLElement(
	evt: UIEvent,
	target: EventTarget | null,
): target is HTMLElement {
	return (
		!!target &&
		(target instanceof HTMLElement || instanceofInWindow(target, evt.win, "HTMLElement"))
	);
}

/** Generalizes Obsidian's onHoverLink to arbitrary callback functions. */
export function onModKeyPress(
...
export function selectDoubleClickedWord(evt: MouseEvent) {
	...
	if (doc.caretRangeFromPoint) {
		pointRange = doc.caretRangeFromPoint(evt.clientX, evt.clientY);
	...
	if (node.nodeType === Node.TEXT_NODE) {
		...
		if (text.length > 0) {
			// Find word boundary manually around the click offset
			// Supports English letters, numbers, accented characters, and Chinese/Japanese/Korean
			const wordRegex = /[\w\u00C0-\u017F\u4e00-\u9fa5\u3040-\u309F\u30A0-\u30FF]/;
			...
			// If we found a non-empty range, select it
			if (start < end) {
				const finalRange = doc.createRange();
				finalRange.setStart(node, start);
				finalRange.setEnd(node, end);
	...
	// Fallback to browser selection if manual expansion didn't yield a result
	selection.removeAllRanges();
	selection.addRange(pointRange);
	selection.modify("move", "backward", "word");
	selection.modify("extend", "forward", "word");

	// Final trim of trailing space
...
export function selectTrippleClickedTextLayerNode(evt: MouseEvent) {
	...
	if (
		!textLayerNode ||
		!textLayerNode.instanceOf(HTMLSpanElement) ||
		!textLayerNode.hasClass("textLayerNode")
	)
		return;

 ...
 */
export function addProductMenuItems(
	rootMenu: Menu,
	itemAdders: ((menu: Menu) => void)[],
	options: { clickableParentItem: boolean; vim: boolean },
) {
		...
		for (const item of newItems) {
			if (item instanceof MenuItem) {
				...
				if (options.vim) {
					...
					const oldEscapeHandler = submenu.scope.keys.find(

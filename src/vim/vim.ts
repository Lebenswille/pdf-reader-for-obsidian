...
export class VimBindings extends PDFReaderComponent {
	...
	get structureParser() {
		if (!this._structureParser && this.pdfViewer && this.viewer.child?.file) {
			this._structureParser = this.addChild(
				new PDFDocumentTextStructureParser(
					this.plugin,
					this.pdfViewer,
					this.viewer.child.file,
				),
			);
		}
	...
	constructor(plugin: PDFReader, viewer: PDFViewerComponent) {
		super(plugin);
		...
		this.vimScope.registerKeymaps(["normal", "visual", "outline"], {
			...
			G: (n) =>
				this.pdfViewer &&
				(this.pdfViewer.currentPageNumber = n ?? this.pdfViewer.pagesCount),
	...
	static register(plugin: PDFReader, viewer: PDFViewerComponent) {

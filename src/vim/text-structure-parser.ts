...
export class PDFDocumentTextStructureParser extends PDFReaderComponent {
	pdfViewer: PDFViewer;
	/** Maps 1-based page numbers to the corresponding page-level parser. */
	pages: Map<number, PDFPageTextStructureParser> = new Map();
	file: TFile;

	constructor(plugin: PDFReader, pdfViewer: PDFViewer, file: TFile) {
		super(plugin);
...
export class PDFPageTextStructureParser {
	...
	getBoundIndicesOfLine(itemIndex: number, lineOffset: number) {
		...
		let lineEndIndex =
			nextLineStartIndex === undefined ? this.items.length - 1 : nextLineStartIndex - 1;
		// Exclude detached EOL element that's replaced with <br>
		while (lineEndIndex > lineStartIndex && !this.items[lineEndIndex].str.length)
			lineEndIndex--;
	...
	_getIndexWithinLineStartIndicesForLineContainingItem(itemIndex: number) {
		...
		const { found, index: lineStartIndex } = binarySearch(

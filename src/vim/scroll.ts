...
export class ScrollController {
	...
	scrollTo(direction: "left" | "right" | "up" | "down", n?: number) {
		...
		let offset = isFirst
			? this.settings.vimScrollSize
			: this.settings.vimContinuousScrollSpeed * this.lastScrollInterval;
		...
		const behavior: ScrollBehavior =
			this.settings.vimSmoothScroll && isFirst ? "smooth" : "instant";
	...
	scrollToTop() {
		...
		if (!pageDiv) return;
		this.viewerContainerEl.scrollTo({
			top: pageDiv.offsetTop,
			behavior: (this.settings.vimSmoothScroll ? "smooth" : "instant") as ScrollBehavior,
		});
	}
	...
	scrollToBottom() {
		...
		if (!pageDiv) return;
		this.viewerContainerEl.scrollTo({
			top: pageDiv.offsetTop + pageDiv.offsetHeight - this.viewerContainerEl.clientHeight,
			behavior: (this.settings.vimSmoothScroll ? "smooth" : "instant") as ScrollBehavior,
		});
	}
	...
	scrollVerticallyByVisualPage(times: number) {
		...
		this.viewerContainerEl.scrollBy({
			top: offset,
			behavior: (this.settings.vimSmoothScroll ? "smooth" : "instant") as ScrollBehavior,
		});
	}

...
import {
	binarySearch,
	binarySearchForRangeStartingWith,
	isTargetHTMLElement,
	isTypable,
	stringCompare,
} from "utils";
...
export class VimScope extends Scope {
	...
	map(modes: string[], fromTo: Record<string, string>) {
		for (const mode of modes) {
			const keymaps = Object.fromEntries(
				Object.entries(fromTo).map(([from, to]) => [
					from,
					(n?: number) => {
						const { found, index } = binarySearch(this.modeToKeymaps[mode], (map) =>
							stringCompare(to, map.keys),
						);
						if (found) {
	...
	handleKey(evt: KeyboardEvent, info: KeymapInfo) {
		...
		(() => {
			...
			const range = binarySearchForRangeStartingWith(
				keymaps,
				keysWithoutNumber,
				(item) => item.keys,
				{

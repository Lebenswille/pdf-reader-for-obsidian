...
export class VimVisualMode extends VimBindingsMode {
	...
	previousSelection: {
		anchor: { page: number; pos: PDFTextPos };
		head: { page: number; pos: PDFTextPos };
	} | null = null;
	...
	getSelectionPos(selection: Selection, which: "anchor" | "head"): PDFTextPos | null {
		...
		const isNodeNonemptyTextDiv = (node: Node): node is HTMLElement => {
			return (
				node.instanceOf(HTMLElement) && node.hasClass("textLayerNode") && !!node.textContent
			);
		};
	...
	extendSelection(
		getNewHeadPos: (state: {
			currentHeadPos: PDFTextPos;
			pageNumber: number;
			pageParser: PDFPageTextStructureParser;
		}) => PDFTextPos | null,
	) {
		const selection = this.doc.getSelection();
		if (!selection) return;
	...
	extendSelectionByLine(lines: number) {
		this.extendSelection(({ currentHeadPos, pageParser }) =>
			pageParser.getLineShiftPosition(currentHeadPos, lines),
		);
	}
	...
	extendSelctionToLineBoundary(forward: boolean) {
		this.extendSelection(({ currentHeadPos, pageParser }) => {
			...
			return forward ? { index: bound.end, offset: -1 } : { index: bound.start, offset: 0 };
		});
	}
	...
	setSelectionByPos(
		...
		if (anchorNodeAndOffset && headNodeAndOffset) {
			selection.setBaseAndExtent(
				anchorNodeAndOffset.node,
				anchorNodeAndOffset.offset,
	...
	rememberSelection() {
		...
		if (selection && !selection.isCollapsed && selection.anchorNode && selection.focusNode) {
			const anchorPage = this.lib.getPageElAssociatedWithNode(selection.anchorNode)?.dataset
				.pageNumber;
			const headPage = this.lib.getPageElAssociatedWithNode(selection.focusNode)?.dataset
				.pageNumber;

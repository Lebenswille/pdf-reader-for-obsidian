...
export class SelectionToolbar extends Component {
    ...
    handleMark(colorName: string | null) {
        ...
        const template = this.plugin.settings.copyCommands[this.plugin.settings.defaultColorPaletteActionIndex].template;
        
        // Ensure we use the cached selection if the current one is lost due to focus
        const commentToUse = (this.comment || '').trim();
        
        // Always use copyLinkToSelection since direct PDF modification is disabled
        this.plugin.lib.copyLink.copyLinkToSelection(false, { copyFormat: template }, colorName ?? undefined, true, commentToUse);
        
        this.hide();
    }

    show(evt: PointerEvent) {
        ...
        if (left < 10) left = 10;
        if (left + toolbarRect.width > win.innerWidth - 10) left = win.innerWidth - toolbarRect.width - 10;

...
export class PDFInternalLinkPostProcessor extends PDFDestinationHolderPostProcessor {
    ...
    protected constructor(plugin: PDFReader, child: PDFViewerChild, linkAnnotationElement: AnnotationElement) {
        super(plugin, child, linkAnnotationElement.container);
        this.linkAnnotationElement = linkAnnotationElement;
    }

    static registerEvents(plugin: PDFReader, child: PDFViewerChild, linkAnnotationElement: AnnotationElement) {
    ...
    async customHover(evt: MouseEvent) {
        if (this.plugin.settings.actionOnCitationHover === 'pdf-reader-bib-popover'

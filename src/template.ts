...
export class TemplateProcessor {
	...
	evalPart(expr: string) {
		// avoid direct eval
		const evaluated = new Function(...Object.keys(this.variables), `return ${expr};`)(
...
export class PDFReaderTemplateProcessor extends TemplateProcessor {
	app: App;
	lib: PDFReaderLib;
	...
	constructor(
		...
		const linkedFileProperties =
			(linkedFile && app.metadataCache.getFileCache(linkedFile)?.frontmatter) ?? {};
	...
	findMarkdownFileAssociatedToPDF(pdf: TFile) {
		...
		const dv = app.plugins.plugins.dataview?.api;
		if (dv) {
			const proxyMDPages: any[] = dv
				.pages()
				.where((page: any) =>
					dv
						.array(page[this.plugin.settings.proxyMDProperty] ?? [])
						.path.includes(pdf.path),
				);
			proxyMDs = proxyMDPages
				.map((page) => app.vault.getAbstractFileByPath(page.file.path))
			...
			for (const sourcePath of backlinks.keys()) {
				...
				if (cache) {
					const isProxyMD = cache.frontmatterLinks?.some((link) => {
						if (
							link.key !== this.plugin.settings.proxyMDProperty &&
							!new RegExp(`${this.plugin.settings.proxyMDProperty}.\\d+`).test(
								link.key,
							)
						) {
							return false;

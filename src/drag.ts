...
export const registerOutlineDrag = async (plugin: PDFReader, pdfOutlineViewer: PDFOutlineViewer, child: PDFViewerChild, file: TFile) => {
    ...
    for (const item of pdfOutlineViewer.allItems) {
        promises.push((async () => {
            ...
            app.dragManager.handleDrop(item.selfEl, (evt, draggable, dragging) => {
                ...
                if (!draggable || draggable.source !== 'pdf-reader' || draggable.type !== 'pdf-offset') return;
...
export const registerAnnotationPopupDrag = (plugin: PDFReader, popupEl: HTMLElement, child: PDFViewerChild, file: TFile, page: number, id: string) => {
    const { app, lib } = plugin;

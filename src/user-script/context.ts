...
export class UserScriptContext extends PDFReaderComponent {
	...
	get page() {
		...
		return (
			(selection && this.lib.copyLink.getPageAndTextRangeFromSelection(selection)?.page) ??
			this.pdfViewer.pdfViewer?.currentPageNumber ??
			null
		);
	}
	...
	evaluateTemplate(copyFormat: string, displayTextFormat: string, color?: string) {
		...
		const subpath = paramsToSubpath({
			page,
			selection: `${selection.beginIndex},${selection.beginOffset},${selection.endIndex},${selection.endOffset}`,
			color: color ? color.toLowerCase() : undefined,
		});
		return this.lib.copyLink.getTextToCopy(
			this.child,
			copyFormat,
			displayTextFormat,
			this.file,

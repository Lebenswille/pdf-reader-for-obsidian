...
export const patchWorkspace = (plugin: PDFReader) => {
		...
		around(Workspace.prototype, {
			openLinkText(old) {
				return function (
					...
					if (
						(plugin.settings.openPDFWithDefaultApp ||
							plugin.settings.singleTabForSinglePDF ||
							plugin.settings.openLinkNextToExistingPDFTab ||
							plugin.settings.paneTypeForFirstPDFLeaf) &&
						!newLeaf
					) {
						// respect `newLeaf` when it's not `false`
						...
						if (file && file.extension === "pdf") {
							if (Platform.isDesktopApp && plugin.settings.openPDFWithDefaultApp) {
								if (
									plugin.settings.openPDFWithDefaultAppAndObsidian &&
									plugin.settings.syncWithDefaultApp
								) {
							...
							if (plugin.settings.singleTabForSinglePDF) {
								const { exists, promise } =
									lib.workspace.openPDFLinkTextInExistingLeafForTargetPDF(
										linktext,
										sourcePath,
										openViewState,
										file,
							...
							if (
								...
								if (pdfLeaf) {
									if (
										...
										return lib.workspace.openPDFLinkTextInLeaf(
											newLeaf,
											linktext,
											sourcePath,
											openViewState,
									...
									const newLeaf = lib.workspace.getLeaf(
										plugin.settings.paneTypeForFirstPDFLeaf,
									);
									return lib.workspace.openPDFLinkTextInLeaf(
										newLeaf,

...
export const patchPagePreview = (plugin: PDFReader): boolean => {
		...
		around(pagePreviewInstance, {
			onLinkHover(old) {
				return function (
					...
					if (
						(!sourcePath || sourcePath.endsWith(".pdf")) &&
						plugin.settings.hoverHighlightAction === "open" &&
						state?.isTriggeredFromBacklinkVisualizer
					) {
						lib.workspace.openMarkdownLinkFromPDF(linktext, sourcePath, false, {
							line: state.scroll,
						});
						return;
					}

...
export const patchBacklink = (plugin: PDFReader): boolean => {
	...
	plugin.register(
		around(Object.getPrototypeOf(backlinkView.constructor.prototype), {
			onLoadFile(old) {
				return async function (this: BacklinkView, file: TFile) {
					...
					if (this.getViewType() === "backlink" && file.extension === "pdf") {
						this.pdfManager = new BacklinkPanePDFManager(
							plugin,
							this.backlink,
							file,
						).setParents(plugin, this);
					}
				};
			},
			onUnloadFile(old) {
	...
	plugin.register(
		around(backlinkRenderer.backlinkDom.constructor.prototype, {
			addResult(old) {
				return function (
					file: TFile,
					...
					if (self.filter) {
						...
						if (cache) {
							...
							for (const [start, end] of result.content) {
								...
								if (linkCache && self.filter(file, linkCache))
									resultFromContent.push([start, end]);
							}

							result.content.length = 0;
							result.content.push(...resultFromContent);

							const resultFromProperties: {
								key: string;
								pos: SearchMatchPart;
								subkey: string[];
							}[] = [];
							...
							for (const item of result.properties) {
								const linkCache = cache.frontmatterLinks?.find(
									(link) => link.key === item.key,
								);
								if (linkCache && self.filter(file, linkCache))
									resultFromProperties.push(item);
							}

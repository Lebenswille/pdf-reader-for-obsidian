...
export class ColorPalette extends PDFReaderComponent {
    ...
    onload() {
        ...
        this.statusContainerEl = this.paletteEl.createDiv('pdf-reader-color-palette-status-container');
    ...
    onItemPointerUp(itemEl: HTMLElement, name: string | null, evt: MouseEvent) {
        ...
        if (this.plugin.settings.syncColorPaletteItem && this.plugin.settings.syncDefaultColorPaletteItem) {
            this.plugin.settings.defaultColorPaletteItemIndex = this.selectedColorName !== undefined ? (name ? (Object.keys(this.plugin.settings.colors).indexOf(name) + 1) : 0) : -1;
        }
    ...
    setActiveItem(name: string | null | undefined) {
        ...
        this.itemEls.forEach((el) => {
            el.toggleClass('is-active', this.selectedColorName === (el.dataset.highlightColor ?? null));
        });
    }
    ...
    applyColorToSelection(name: string | null) {
        ...
        // Always use copyLinkToSelection since direct PDF modification is disabled
        this.lib.copyLink.copyLinkToSelection(false, { copyFormat: template }, name ?? undefined, true, comment);

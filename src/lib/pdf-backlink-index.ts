...
import { PDFReaderComponent } from "./component";
import { MultiValuedMap } from "utils";

export class PDFBacklinkIndex extends PDFReaderComponent {
	file: TFile;
	...
	onload() {
		this.init();
		this.registerEvent(
			this.app.metadataCache.on("changed", (sourceFile, _, cache) => {
		...
		this.registerEvent(
			this.app.metadataCache.on("deleted", (sourceFile) => {
		...
		this.registerEvent(
			this.app.vault.on("rename", (sourceFile, oldSourcePath) => {
...
export class PDFPageBacklinkIndex {
	...
	static selectionIdToParams(id: string): NonNullable<PDFBacklinkCache["_selection"]> {
		const [beginIndex, beginOffset, endIndex, endOffset] = id
			.split(",")
			.map((s) => parseInt(s));
...
export class PDFBacklinkCache {
	...
	set selection(selection: PDFBacklinkCache["_selection"]) {
		...
		if (pageIndex) {
			if (this.selection) {
				pageIndex?.selections.deleteValue(
					PDFPageBacklinkIndex.selectionId(this.selection),
					this,
				);
			}
			if (selection) {

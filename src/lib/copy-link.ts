...
export class copyLinkLib extends PDFReaderLibSubmodule {
    ...
    getTemplateVariables(subpathParams: Record<string, any>) {
        ...
        if ((!selection || !selection.toString()) && this.lastSelectionVariables) {
             const existingParams = Object.fromEntries(subpathToParams(this.lastSelectionVariables.subpath).entries());
             return { ...this.lastSelectionVariables, subpath: paramsToSubpath({
                 ...existingParams,
                 ...subpathParams
             })};
        }
    ...
    copyLinkToSelection(checking: boolean, templates: { copyFormat: string, displayTextFormat?: string }, colorName?: string, autoPaste?: boolean, comment?: string): boolean {
        ...
        if (variables) {
            ...
            if (!checking) {
                (async () => {
                    const evaluated = this.getTextToCopy(child, templates.copyFormat, templates.displayTextFormat, file, page, subpath, text, colorName?.toLowerCase() ?? '', undefined, comment);
    ...
    copyLinkToAnnotation(child: PDFViewerChild, checking: boolean, templates: { copyFormat: string, displayTextFormat?: string }, page: number, id: string, autoPaste?: boolean, shouldShowStatus?: boolean, comment?: string): boolean {
        const file = child.file;
        ...
        if (!checking) {
            ...
            child.getAnnotatedText(pageView, id)
                .then(async (text) => {
                    ...
                    const evaluated = this.getTextToCopy(child, templates.copyFormat, templates.displayTextFormat, file, page, subpath, text ?? '', color, undefined, comment);
    ...
    copyLinkToAnnotationWithGivenTextAndFile(text: string, file: TFile, child: PDFViewerChild, checking: boolean, templates: { copyFormat: string, displayTextFormat?: string }, page: number, id: string, colorName: string, autoPaste?: boolean, comment?: string) {
        if (!checking) {
            (async () => {
                const evaluated = this.getTextToCopy(child, templates.copyFormat, templates.displayTextFormat, file, page, `#page=${page}&annotation=${id}`, text, colorName, undefined, comment);
    ...
    writeHighlightAnnotationToSelectionIntoFileAndCopyLink(checking: boolean, templates: { copyFormat: string, displayTextFormat?: string }, colorName?: string, autoPaste?: boolean, comment?: string): boolean {
        // Commented out because it used lib.highlight.writeFile
        /*
            ...
            this.lib.highlight.writeFile.addTextMarkupAnnotationToSelection(
                ...
                .then((result) => {
                    ...
                    setTimeout(() => {
                        ...
                        this.copyLinkToAnnotationWithGivenTextAndFile(text, file, child, false, templates, page, annotationID, `${r}, ${g}, ${b}`, autoPaste, comment);
        ...
        */
        ...
        new Notice('Writing annotations to file is currently disabled.');
        return true;
    }
    ...
    async getAutoFocusOrAutoPasteTarget(target: AutoFocusTarget): Promise<TFile | null> {
        ...
        if (target === 'last-paste') targetFile = lastPasteFile;
            ...
            const activePDFView = this.lib.workspace.getActivePDFView();
            if (activePDFView && activePDFView.file) {
                targetFile = await this.lib.sync.getNoteFileFromPDF(activePDFView.file);
            }
        }
    ...
    async prepareMarkdownLeafForPaste(file: TFile) {
        ...
        const shouldOpen = !leaf && this.settings.openAutoFocusTargetIfNotOpened && !this.settings.autoSync;

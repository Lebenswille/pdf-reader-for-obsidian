...
abstract class NameOrNumberTree<
	...
	TreeNode extends NameOrNumberTreeNode<Key>,
> {
	_rootDict: PDFDict;
...
abstract class NameOrNumberTreeNode<Key extends string | number> {
	...
	sortKids() {
		...
		kids.sort((a, b) => {
			...
			return aLimits[0] < bLimits[0] ? -1 : aLimits[0] > bLimits[0] ? 1 : 0;
		});
	...
	iter(callbacks: {
		enter?: (node: NameOrNumberTreeNode<Key>) => any;
		leave?: (node: NameOrNumberTreeNode<Key>) => any;
	}) {

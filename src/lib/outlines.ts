...
export class PDFOutlines {
	...
	constructor(plugin: PDFReader, doc: PDFDocument) {
		this.plugin = plugin;
		this.doc = doc;
		this.namedDests = PDFNamedDestinations.fromDocument(doc);
	}

	static async fromDocument(doc: PDFDocument, plugin: PDFReader) {
		return new PDFOutlines(plugin, doc);
	}

	static async fromFile(file: TFile, plugin: PDFReader) {
		const { lib } = plugin;
	...
	// TODO
	// static async fromMarkdownList(markdown: string, plugin: PDFReader, doc: PDFDocument) {
	...
	iter(callbacks: {
		enter?: (item: PDFOutlineItem) => any;
		leave?: (item: PDFOutlineItem) => any;
	}) {
	...
	async iterAsync(callbacks: {
		enter?: (item: PDFOutlineItem) => Promise<any>;
		leave?: (item: PDFOutlineItem) => Promise<any>;
	}) {
			...
			enter: async (outlineItem) => {
				...
				if (node.item.title === outlineItem.title) {
					...
					if (typeof dest === "string") {
						...
						if (
							JSON.stringify(this.lib.normalizePDFJsDestArray(dest, pageNumber)) ===
							JSON.stringify(outlineDest)
						) {
							found = outlineItem;
						}
					}
	...
	static async processOutlineRoot(
		process: (root: PDFOutlineItem) => void,
		file: TFile,
		plugin: PDFReader,
	) {
		const { app } = plugin;
	...
	static async findAndProcessOutlineItem(
		item: PDFOutlineTreeNode,
		processor: (item: PDFOutlineItem) => void,
		file: TFile,
		plugin: PDFReader,
	) {
		const { app } = plugin;
...
export class PDFOutlineItem {
	...
	async sortChildren() {
		...
		await this.iterChildrenAsync(async (child) => {
			...
			const top =
				destArray[1] === "XYZ"
					? destArray[3]
					: destArray[1] === "FitBH" || destArray[1] === "FitH"
						? destArray[2]
						: undefined;
	...
	async destNotExistInDoc(): Promise<boolean> {
		...
		throw new Error(
			"The first element of a destination array must be a refernece of a page leaf node.",
		);
	}

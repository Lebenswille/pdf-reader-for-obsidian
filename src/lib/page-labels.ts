...
export class PDFPageLabels {
	...
	constructor(
		...
		public ranges: { pageFrom: number; dict: PDFPageLabelDict }[],
	) {
		this.normalize();
	}
		...
		if (this.ranges.length) {
			...
			for (let i = this.ranges.length - 1; i >= 0; i--) {
				...
				if (
					...
					if (
						range.pageFrom - prevRange.pageFrom ===
						range.dict.start - (prevRange.dict.start ?? 1)
					) {
						this.ranges.splice(i, 1);
						continue;
					}
	...
	divideRangeAtPage(
		page: number,
		keepLabels: boolean,
		processDict?: (newDict: PDFPageLabelDict) => void,
	) {

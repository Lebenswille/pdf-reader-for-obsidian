...
export class HighlightGeometryLib extends PDFReaderLibSubmodule {
	/**
	...
	computeMergedHighlightRects(
		textLayer: { textDivs: HTMLElement[]; textContentItems: TextContentItem[] },
		beginIndex: number,
		beginOffset: number,
		endIndex: number,
		endOffset: number,
		...
		for (let index = beginIndex; index <= endIndex; index++) {
			...
			const rect = this.computeHighlightRectForItem(
				item,
				textDiv,
				index,
				beginIndex,
	...
	computeHighlightRectForItem(
		...
		if (item.chars && item.chars.length >= item.str.length) {
			return this.computeHighlightRectForItemFromChars(
				item as PropRequired<TextContentItem, "chars">,
				index,
				beginIndex,
				beginOffset,
				endIndex,
		...
		return this.computeHighlightRectForItemFromTextLayer(
			item,
			textDiv,
			index,
			beginIndex,
	...
	computeHighlightRectForItemFromChars(
		...
		const offsetTo =
			(index === endIndex ? Math.min(endOffset, trimmedChars.length) : trimmedChars.length) -
			1;
		...
		// the minimum rectangle that contains all the chars of this text content item
		return [
			Math.min(charFrom.r[0], charTo.r[0]),
			Math.min(charFrom.r[1], charTo.r[1]),
			Math.max(charFrom.r[2], charTo.r[2]),
			Math.max(charFrom.r[3], charTo.r[3]),
		];
	}

	computeHighlightRectForItemFromTextLayer(
		item: TextContentItem,
		textDiv: HTMLElement,
		index: number,
		beginIndex: number,
	...
	areRectanglesMergeable(rect1: Rect, rect2: Rect): boolean {
		return (
			this.areRectanglesMergeableHorizontally(rect1, rect2) ||
	...
	areRectanglesMergeableVertically(rect1: Rect, rect2: Rect): boolean {
		...
		return (
			Math.abs(left1 - left2) < threshold &&
			Math.abs(right1 - right2) < threshold &&
			height1 / width1 > 0.85 &&
			height2 / width2 > 0.85
		);
	}
	...
	mergeRectangles(...rects: Rect[]): Rect {
		...
		return [Math.min(...lefts), Math.min(...bottoms), Math.max(...rights), Math.max(...tops)];
	}

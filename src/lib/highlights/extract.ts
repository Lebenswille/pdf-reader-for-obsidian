...
type AnnotatedTextsInPage = Map<string, { text: string, rgb: RGB | null, comment?: string, rects: Rect[] }>;
...
export class HighlightExtractor extends PDFReaderLibSubmodule {
    ...
    async getAnnotatedTextsInPage(page: PDFPageProxy): Promise<AnnotatedTextsInPage> {
        ...
        const results: { id: string, textRanges: PDFTextRange[], rgb: RGB | null, comment?: string, left: number, top: number, rects: Rect[] }[] = [];
        ...
        for (const annot of annots) {
            ...
            results.push({ id: annot.id, textRanges, rgb, comment, left: rects[0][0], top: rects[0][3], rects });
        }
        ...
        return new Map(
            results
                ...
                .map((result) => {
                    ...
                    return [result.id, { text, rgb: result.rgb, comment: result.comment, rects: result.rects }];
                })
        );
    }

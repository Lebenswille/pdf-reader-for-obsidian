...
type AnnotatedTextsInPage = Map<
	string,
	{ text: string; rgb: RGB | null; comment?: string; rects: Rect[] }
>;
...
export class HighlightExtractor extends PDFReaderLibSubmodule {
	...
	async getAnnotatedTextsInPage(page: PDFPageProxy): Promise<AnnotatedTextsInPage> {
		...
		for (const annot of annots) {
			const isTextMarkupAnnot = ["Highlight", "Underline", "Squiggly", "StrikeOut"].includes(
				annot.subtype,
			);
			...
			const textRanges = rects.map((rect) =>
				this.getTextByRect(items as TextContentItem[], rect),
			);

			const rgb = annot.color
				? ({ r: annot.color[0], g: annot.color[1], b: annot.color[2] } as RGB)
				: null;
		...
		return new Map(
			results
				...
				.map((result) => {
					let text = result.textRanges.map((range) => range.text).join("\n");
					text = this.lib.toSingleLine(text);
					return [
						result.id,
						{ text, rgb: result.rgb, comment: result.comment, rects: result.rects },
					];
				}),
		);
	}

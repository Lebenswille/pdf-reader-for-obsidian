...
export class PDFReaderSync extends PDFReaderLibSubmodule {
    ...
    private registerPDFOpenListener() {
        this.plugin.registerEvent(this.app.workspace.on('file-open', async (file) => {
            if (file instanceof TFile && file.extension === 'pdf') {
                await this.ensureNoteForPDF(file);
            }
        }));
    }

    async ensureNoteForPDF(pdfFile: TFile) {
        const noteFile = await this.getNoteFileFromPDF(pdfFile);
        if (!noteFile) {
            await this.createNoteForPDF(pdfFile);
        }
    }

    async createNoteForPDF(pdfFile: TFile) {
        ...
        for (const file of files) {
            ...
            for (const propName of propNames) {
                ...
                if (cache?.frontmatterLinks) {
                    const fmLink = cache.frontmatterLinks.find((l: any) => l.key.split('.')[0] === propName);
    ...
    private async refreshHeaderButton() {
        ...
        if (!(view instanceof MarkdownView) || !view.file) {
            ...
            view.containerEl.querySelectorAll(`.${PDFReaderSync.HEADER_BTN_CLASS}, .view-action[aria-label^="Open source PDF"]`).forEach((el: Element) => el.remove());
            return;
        }
        ...
        parentEl.querySelectorAll(`.${PDFReaderSync.HEADER_BTN_CLASS}, .view-action[aria-label^="Open source PDF"]`).forEach((el: Element) => el.remove());
    ...
    async syncAnnotations() {
        ...
        if (!pdfFile) {
            ...
            if (pdfView && (pdfView as any).viewer?.child?.file) {
                ...
                noteFile = await this.getNoteFileFromPDF(pdfFile!);
            }
        }
    ...
    async syncFromPDFToNote(pdfFile: TFile, noteFile: TFile) {
        ...
        try {
            ...
            const copyCommand = this.settings.copyCommands[this.settings.defaultColorPaletteActionIndex] || this.settings.copyCommands[0];
            ...
            for (const pageNumber of pages) {
                ...
                for (const [id, info] of resultsInPage) {
                    ...
                    if (!content.includes(annotIDQuery)) {
                        const helperChild = child || (this.app.workspace.getLeavesOfType('pdf').first()?.view as any)?.['viewer']?.['child'];
                        let callout = this.lib.copyLink.getTextToCopy(
                            helperChild, template, undefined, pdfFile, pageNumber,
                            `#page=${pageNumber}&annotation=${id}`,
                            text, color, undefined, comment
                        );

...
export class DummyFileManager extends PDFReaderLibSubmodule {
	async createDummyFilesInFolder(folderPath: string, uris: string[]) {
		...
		if (!folderExists) {
			try {
				...
				console.error(
					`${this.plugin.manifest.name}: Failed to create folder "${folderPath}" due to the following error: `,
					error,
				);
				return [];
			}
		...
		return await Promise.all(
			uris.map(async (uri) => {
				...
				try {
					...
					console.error(
						`${this.plugin.manifest.name}: Failed to create a dummy file "${availableFilePath}" due to the following error: `,
						error,
					);
					throw error;
				}
	...
	async createDummyFilesOnEditorDrop(
		evt: DragEvent,
		editor: Editor,
		...
		if (uris.length) {
			...
			dummyFiles.forEach((dummyFile, index) => {
				let text = this.app.fileManager.generateMarkdownLink(
					dummyFile,
					info.file?.path ?? "",
				);
	...
	getUrisFromDataTransfer(dataTransfer: DataTransfer) {
		if (window.electron) {
			// the file path is available only in the desktop app
			const droppedFiles = Array.from(dataTransfer.files);
			if (
				droppedFiles.length &&
				droppedFiles.every((file) => file.type === "application/pdf")
			) {
		...
		const draggedUris = dataTransfer
